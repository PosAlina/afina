...
bool SimpleLRU::Put(const std::string &key, const std::string &value) {
	...
	if (_lru_index.find(key) != _lru_index.end()) return UpdateNode(key, value); // There is already such a key.
...
bool SimpleLRU::Delete(const std::string &key) {
	...
	if (current_node->next != nullptr) { // It is not the last node.
		//Change prev node.
		if (current_node->prev == nullptr) _lru_head = std::move(current_node->next); // It is first node.
		else current_node->prev->next = std::move(current_node->next); //It is not first node.
